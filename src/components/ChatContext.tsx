...
interface ChatContextValue {
  ...
  createSession: (modelId?: string, zygId?: string | null) => void;
  deleteSession: (id: string) => void;
  updateMessage: (sessionId: string, messageId: string, content: string, reasoning?: string) => void;
...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) =>
  {
   ...
   const [error, setError] = useState<string | undefined>(undefined);
    ...
    const createSession = useCallback((modelId?: string, zygId?: string | null) => {
...
const renameSession = useCallback((id: string, title: string) => {
  ...
  const updateMessage = useCallback((sessionId: string, messageId: string, content: string, reasoning?: string) => {
    ...
    const updateSessionZyg = useCallback((sessionId: string, zygId: string | null) => {
      setSessions(prev => prev.map(s => s.id === sessionId ? { ...s, zygId, updatedAt: new Date().toISOString() } as any : s));
    }, []);
...
const appendMessage = useCallback((sessionId: string, message: Message) => {
    try {
      ...
      // Still add the message even if cleaning fails
      setSessions(prev => prev.map(session => {
        if (session.id !== sessionId) return session;
        return {
          ...session,
...
const streamStart = performance.now();
            ...
            } as any,
            {
              onChunk: (chunk) => {
               ...
               onReasoningChunk: (chunk) => {
                 console.log('onReasoningChunk called:', chunk.slice(0, 30));
              ...
              onComplete: (fullMessage) => {
                const finalContent = searchResultsText
                  ...
                  : fullMessage;
                updateMessage(session.id, aiMessageId, finalContent);
                streamingBuffer.current = '';
                reasoningBuffer.current = '';
      ...
      }, [activeSessionId, sessions, models, settings, createSession, updateSession, appendMessage, updateMessage, renameSession]);
